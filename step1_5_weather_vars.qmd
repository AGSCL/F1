...
ordered<- 
paste(rep(c("control","treatment"), each = length(2000:2022)),rep(2000:2022, times = 2), sep=".")
ordered_with_placebo_years<- 
c("control.2000", "control.2001", "control.2002", "control.2003", 
"control.2004", "control.2005", "control.2006", "control.2007", "control.2008", 
"control.2009", "control.2010", "control.2011", "control.2012", "control.2013", 
"control.2014", "control.2015", "control.2016", "control.2017", "control.2018", 
"control.2019", "control.2020", "control.2021", "control.2022", "treatment.2009", 
"treatment.2020", "treatment.2021", "treatment.2000", "treatment.2001", "treatment.2002", 
"treatment.2003", "treatment.2004", "treatment.2005", "treatment.2006", 
"treatment.2007", "treatment.2008", "treatment.2010", "treatment.2011", 
"treatment.2012", "treatment.2013", "treatment.2014", "treatment.2015", 
"treatment.2016", "treatment.2017", "treatment.2018", "treatment.2019", 
 "treatment.2022")


collisions_weather_corr_rect_synth <-
  collisions_weather_corr_rect |>
  filter(date_num <= unclass(race_date_rec) + 7) |>
  ...
  mutate(
    ...
    id       = as.numeric(factor(paste0(tr_contr_sens, ".",year.x), levels= ordered_with_placebo_years)),
    id_lab   = factor(paste0(tr_contr_sens, ".",year.x))
  ) |>
  select(-year.y) |> 
  mutate(id_lab= case_when(id_lab== "treatment.2009"~ "placebo.2009",
                        id_lab== "treatment.2020"~ "placebo.2020",
                        id_lab== "treatment.2021"~ "placebo.2021",
                        T~id_lab)) |> 
  mutate(tr_contrl_corr= ifelse(grepl("treatment",id_lab),1,0))
...
cat("Day of the race \n")
collisions_weather_corr_rect_synth |> 
  filter(race_day==1) |> reframe(min=yday_corr, max=yday_corr) |>  print(n=46)
...
collisions_weather_corr_rect_synth_tr <- 
  ...
  filter(grepl("treatment", id_lab)) %>%            # keep treated rows only
  group_by(yday_corr) %>%              # collapse across MRC × years
  summarise(
    sum_velocidad = sum(sum_velocidad, na.rm = TRUE),
    sum_velocidad_lead1= sum(sum_velocidad_lead1, na.rm = T),
    sum_velocidad_lead2= sum(sum_velocidad_lead2, na.rm = T),    
  ...
  mutate(
    ...
    race_day = 0L,
    id       = 46                          # single ID for the synthetic treated series
  )
...
rbind.data.frame(collisions_weather_corr_rect_synth_cntr,collisions_weather_corr_rect_synth_tr |> 
  ...
  mutate(treated_cntrl= ifelse(id==46, "treated","control"))
...
collisions_weather_corr_rect_quebec_synth <-
  ...
  mutate(
    race_day = if_else(!is.na(year.y), 1L, 0L),
    id       = as.numeric(factor(paste0(tr_contr, ".",year.x), levels= ordered_with_placebo_years)),
    id_lab   = factor(paste0(tr_contr, ".",year.x))
  ) |>
  select(-year.y) |> 
  mutate(id_lab= case_when(id_lab== "treatment.2009"~ "placebo.2009",
                           id_lab== "treatment.2020"~ "placebo.2020",
...
collisions_weather_corr_rect_quebec_synth_cntr <- 
  collisions_weather_corr_rect_quebec_synth %>% 
  filter(!grepl("treatment", id_lab)) %>%            # keep treated rows only
  group_by(yday_corr) %>%            # collapse across all control IDs
  summarise(
    sum_velocidad = sum(sum_velocidad, na.rm = TRUE),
    sum_velocidad_lead1= sum(sum_velocidad_lead1, na.rm = T),
    sum_velocidad_lead2= sum(sum_velocidad_lead2, na.rm = T),         
...
After the second meeting with Ignacio, we decided to compare exclusivelly one week with the race. So we selected thursday to sunday dates.
...
#| warning: true
#| error: true
#| label: filtered-db-mega-collapsed
#| results: "hold"


collisions_weather_corr_rect_pre_post <-
  collisions_weather_corr_rect |>
  #4 days , 2 weeks before
    mutate(
    pre_exposure_post_conv = coalesce(
      as.integer(dplyr::between(date, race_date_rec - days((14+3)),
                                 race_date_rec - days((14-0)))), 0L),
      exposure_post_conv = coalesce(
      as.integer(dplyr::between(date, race_date_rec - days(3),
                                 race_date_rec + days(0))), 0L),
...
# collisions_weather_corr_rect_pre_post |> 
    # filter(pre_exposure_post_conv==1) |> 
    # group_by(cluster) |> 
    # summarise(min=min(date), max= max(date))

collisions_weather_corr_rect_pre_post_full <-
  collisions_weather_corr_rect |>
  #4 days , 2 weeks before
    mutate(
    pre_exposure_post_conv = coalesce(
...
rbind.data.frame(
head(collisions_weather_corr_rect_pre_post_full |> dplyr::select(yday_corr, date, race_date_rec, pre_exposure_post_conv, exposure_post_conv, neg_exposure_post_conv) |> mutate(wd=weekdays(date)), 13),
tail(collisions_weather_corr_rect_pre_post_full |> dplyr::select(yday_corr, date, race_date_rec, pre_exposure_post_conv, exposure_post_conv, neg_exposure_post_conv) |> mutate(wd=weekdays(date)), 13)
)|> knitr::kable("markdown", caption= "First and last 13 days of the database with different exposure windows")

# ── 1.  load package ------------------------------------------
library(tableone)

# ── 2.  identify the variables by distribution -----------------
nonnormal <- c("rate_veh",
               "rate_lic",
               "rate_lead1_veh",
               "rate_lead1_lic",
               "rate_lead2_veh",
               ...
               "q1_total_precip_median_lin",
               "q3_total_precip_median_lin")

normal <- c("mean_min_temp_mean_lin",
            "sd_min_temp_mean_lin",
            "mean_max_temp_mean_lin",
            "sd_max_temp_mean_lin")

vars <- c(nonnormal, normal)

# ── 4.  create Table 1 -----------------------------------------
#Exposure.FALSE Exposure.TRUE Post-exposure.FALSE Post-exposure.TRUE Pre-exposure.FALSE Pre-exposure.TRUE 
tab1 <- CreateTableOne(vars      = vars,
                       strata    = "D",
                       data      = subset(collisions_weather_corr_rect_pre_post, grepl("Pre",exposure_window)),
                       test      = TRUE)

# ── 5.  print / export -----------------------------------------
print(tab1, smd = TRUE, nonnormal= nonnormal, quote = TRUE, noSpaces = TRUE)           # console
# write.csv(print(tab1), "table1.csv")   # if you want a CSV
...
tab1_df <- print(
  ...
  noSpaces     = TRUE,    # sin espacios extra
  printToggle  = FALSE    # devuelve lo que imprime en vez de cat()
) |>
  as.data.frame(stringsAsFactors = FALSE) |>
  rownames_to_column(var = "Term")        # pasa rownames a columna
names(tab1_df[c(1:4,6)]) <- c("Term", "Non-host MRC-years", "Host MRCs-years", "Sig.", "SMD")
tab1_df[,4:5]<-NULL

tab1_sens <- CreateTableOne(vars      = vars,
                       strata    = "D_sens",
                       data      = subset(collisions_weather_corr_rect_pre_post, grepl("Pre",exposure_window)),
                       test      = TRUE)
...
tab1_sens_df <- print(
  ...
  as.data.frame(stringsAsFactors = FALSE) |>
  rownames_to_column(var = "Term")        # pasa rownames a columna
names(tab1_sens_df[c(1:4,6)]) <- c("Term", "Non-host MRC-years", "Host MRCs-years", "Sig.", "SMD")
tab1_sens_df[,4:5]<-NULL


cbind.data.frame(type=c("Controls: Quebec only", rep("", 14),"Controls: Quebec+Sherbrooke", rep("", 14)),
                 rbind.data.frame(
                   tab1_df,
                   tab1_sens_df
                 )
                 ) |> 
  knitr::kable("markdown", caption="Pre-exposure, MRCs-years affected by the race vs. non-host or in years 2009,2020, 2021")
# cbind.data.frame(type=c("Controls: Quebec only", rep("", 14),"Controls: Quebec+Sherbrooke", rep("", 14)),
#                  rbind.data.frame(
#                    tab1_df,
#                    tab1_sens_df
#                  )
#                  ) |> filter(!grepl("_veh",Term)) |> rio::export("clipboard")
```
...
acf(collisions_weather_corr_rect_synth_tr$rate_veh,  lag.max = 50, main = "Treatments\nACF:high-speed tickets per 1MM vehicles")
pacf(collisions_weather_corr_rect_synth_tr$rate_veh, lag.max = 50, main = "Treatments\nPACF:high-speed tickets per 1MM vehicles")
```
...
x_vals <- time(forecast::msts(
    collisions_weather_corr_rect_synth_cntr$rate_veh,
    start = c(2019, 1),
    seasonal.periods = c(7)
))


forecast::autoplot(forecast::mstl(forecast::msts(collisions_weather_corr_rect_synth_tr$rate_veh*10,start = c(2019, 1),seasonal.periods = c(7)), robust = TRUE))+ ggtitle("Treated (per10M)")+
  scale_x_continuous(
    breaks = seq(2019, 2025, by = 1/7)[seq(1, 43, by = 7)],          # where the ticks sit
    labels = seq(-35, 7)[seq(1, 43, by = 7)]         # what the ticks say
  ) +
  geom_vline(
    xintercept = seq(2019, 2025, by = 1/7)[which.min(abs(seq(-abs(min_value)+1, 7)))],   # rel_day 0
    linetype   = "dashed", colour = "black")
  

forecast::autoplot(forecast::mstl(forecast::msts(collisions_weather_corr_rect_synth_cntr$rate_veh*10,start = c(2019, 1),seasonal.periods = c(7)), robust = TRUE))+ ggtitle("Controls (per 10M)")+
    scale_x_continuous(
    breaks = seq(2019, 2025, by = 1/7)[seq(1, 43, by = 7)],          # where the ticks sit
    labels = seq(-35, 7)[seq(1, 43, by = 7)]         # what the ticks say
  ) +
...
ggplot(collisions_weather_corr_rect_synth_cntr_tr, aes(x = yday_corr, y = sum_velocidad)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = abs(min_value)+1, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Number of High‑speed Tickets (sum)") +
  theme_minimal() +
  theme(panel.grid.minor = element_blank(), legend.position = "bottom") 


ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = sum_velocidad)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = abs(min_value)+1, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Number of High‑speed Tickets (sum)") +
...
ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = (sum_velocidad/exp_veh)*1e6)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = 30, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Number of Daily high-speed collisions per 1MM vehicle counts (sum)") +
...
ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = mean_min_temp_mean_lin)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = 30, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Min temperature") +
...
ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = mean_max_temp_mean_lin)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = 30, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Max temperature") +
...
ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = mean_median_total_precip_median_lin)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = 30, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-30 to 5 days post-treatment)", y = "Precipitations \n(annual mean of city-level medians, where each city median\nis computed from the medians of its weather stations)") +
...
ggplot(collisions_weather_corr_rect_quebec_synth_cntr_tr, aes(x = yday_corr, y = mean_median_lag_2_prec_median_imp)) +
  geom_line(aes(group = id, colour = treated_cntrl)) +
  scale_colour_brewer(palette = "Dark2", name = "MRC") +
  geom_vline(xintercept = 38, linetype = "dashed", color = "black", linewidth=1) +
  labs(x = "Days (-38 to 5 days post-treatment)", y = "Precipitations (2-day lagged)\n(annual mean of city-level medians, where each city median\nis computed from the medians of its weather stations)") +
    geom_ribbon(
    data = collisions_weather_corr_rect_quebec_synth_cntr,
    aes(x = yday_corr, y = mean_median_lag_2_prec_median_imp,
        ymin = p25_lag_2_prec_median_imp,
        ymax = p75_lag_2_prec_median_imp),
    inherit.aes = FALSE,
