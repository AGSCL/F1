...
After the second meeting with Ignacio, we decided to compare exclusivelly one week with the race. So we selected 
...
collisions_weather_corr_rect_pre_post <-
  ...
  #4 days , 2 weeks before
    ...
    pre_exposure_post_conv = coalesce(
      as.integer(dplyr::between(date, race_date - days((14+3)),
                                 race_date - days((14-0)))), 0L),
      exposure_post_conv = coalesce(
      as.integer(dplyr::between(date, race_date - days(3),
                                 race_date + days(0))), 0L),
  ...
  mutate(                                     # ── aux variables
    ...
    rate_lead1_veh = sum_velocidad_lead1 / exp_veh * 1e6, # not to model
    rate_lead1_lic = sum_velocidad_lead1 / exp_lic * 1e6, # not to model    
    rate_lead2_veh = sum_velocidad_lead1 / exp_veh * 1e6, # not to model
    rate_lead2_lic = sum_velocidad_lead1 / exp_lic * 1e6, # not to model        
    off_veh  = log(exp_veh),                  # offset
    off_lic  = log(exp_lic)
  ) |> 
...
tab1_exp <- CreateTableOne(vars      = vars,
                       strata    = "D",
                       data      = subset(collisions_weather_corr_rect_pre_post, grepl("^Exposure",exposure_window)),
                       test      = TRUE)

# ── 5.  print / export -----------------------------------------
print(tab1_exp, smd = TRUE, nonnormal= nonnormal, quote = TRUE, noSpaces = TRUE)           # console# write.csv(print(tab1), "table1.csv")   # if you want a CSV

tab1_post <- CreateTableOne(vars      = vars,
                       strata    = "D",
                       data      = subset(collisions_weather_corr_rect_pre_post, grepl("^Exposure",exposure_window)),
                       test      = TRUE)

# ── 5.  print / export -----------------------------------------
print(tab1_post, smd = TRUE, nonnormal= nonnormal, quote = TRUE, noSpaces = TRUE)           # console# write.csv(print(tab1), "table1.csv")   # if you want a CSV
```
